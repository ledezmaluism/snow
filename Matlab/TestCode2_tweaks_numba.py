...
NFFT = 2**8; # number of fft points/ number of discretizations of the fast time axis
...
def sweep_dT(dT):
    ...
    pavg=pin;# average pump power
    p = 0.88*pavg/(frep*tw*1e-15); #calculation of peak pump power for sech pulse
    seed=sqrt(p)/np.cosh(tau/tw*1.76); # input pump profile
...
tic = time.time()
dT_array = np.array([3, 4, 5]); # detuning in fs
for dT in dT_array:
    a,b,A,B,data_a = sweep_dT(dT)
